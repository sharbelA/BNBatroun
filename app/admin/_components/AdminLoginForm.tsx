...
import { adminLoginAction } from '@/app/_actions/auth'
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      ...
      <button
        type="submit"
        disabled={isPending}
        className="mt-1 h-11 rounded-xl bg-warm-900 text-sm font-semibold text-white transition hover:bg-warm-800 disabled:opacity-60"
      >
        {isPending ? 'Verifying…' : 'Sign in to admin'}
      </button>
    </form>
  )
}

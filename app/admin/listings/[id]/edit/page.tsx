...
export default async function EditListingPage(
  ...
  // TODO: fetch listing by id from Supabase and pre-fill form values
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="mb-8 text-2xl font-semibold text-warm-900">Edit chalet</h1>

      {/* TODO: wire up to a Server Action that calls supabase.from('listings').update(...) */}
      <form className="flex flex-col gap-8">
        ...
        <section className="flex flex-col gap-4">
          ...
          <Field label="Slug" htmlFor="slug" hint="Changing the slug breaks existing links">
            <input id="slug" name="slug" type="text" required className={inputCls} />
          </Field>
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <Field label="Price / night ($)" htmlFor="price">
              <input id="price" name="price" type="number" min={1} step="0.01" required className={inputCls} />
            </Field>
            <Field label="Bedrooms" htmlFor="bedrooms">
              <input id="bedrooms" name="bedrooms" type="number" min={0} required className={inputCls} />
            </Field>
            <Field label="Bathrooms" htmlFor="bathrooms">
              <input id="bathrooms" name="bathrooms" type="number" min={0} required className={inputCls} />
            </Field>
            <Field label="Max guests" htmlFor="max_guests">
              <input id="max_guests" name="max_guests" type="number" min={1} required className={inputCls} />
            </Field>
          </div>
        </section>
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {AMENITIES.map(({ name, label }) => (
              <label key={name} className="flex cursor-pointer items-center gap-2 text-sm text-warm-700">
                <input type="checkbox" name={name} className="h-4 w-4 rounded border-sand-300 accent-sea-600" />
                {label}
              </label>
            ))}
          </div>
        ...
        <section className="flex flex-col gap-4">
          ...
          <Field label="Rules" htmlFor="house_rules" hint="One rule per line">
            <textarea
              id="house_rules"
              name="house_rules"
              rows={4}
        ...
        <section className="flex flex-col gap-3">
          ...
          <label className="flex cursor-pointer items-center gap-3 text-sm text-warm-700">
            <input type="checkbox" name="is_active" className="h-4 w-4 rounded border-sand-300 accent-sea-600" />
            Active (visible to guests)
          </label>
          <label className="flex cursor-pointer items-center gap-3 text-sm text-warm-700">
            <input type="checkbox" name="is_featured" className="h-4 w-4 rounded border-sand-300 accent-sea-600" />
      ...
      <section className="mt-16 rounded-xl border border-red-200 bg-red-50 p-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            type="button"
            className="h-9 rounded-lg border border-red-300 bg-white px-4 text-sm font-medium text-red-700 transition hover:bg-red-100"
          >
            Deactivate chalet
          </button>
          <button
